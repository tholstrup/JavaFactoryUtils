 ...
 * <p />
 * If caching is enabled this class will by default key the cache by {@link MethodSignature} objects. If you wish to provide a different caching key
 * you may override the {@link BaseParameterizedFactory#getCacheKey(Class[], Object[])} method in this class. If you need to override the key using
 * other parameters you will need to create your own factory.
 * <p />
 ...
 * @see MethodSignature
 */
public abstract class BaseParameterizedFactory extends BaseFactory {
	 ...
	 * suggestion might be a SoftHashMap of which there are freely available implementations on the net and more sophisticated LRU and LFU caches
	 * which may have implementations available.
	 * <p />
	 * It should be noted that the current default key implementation (MethodSignature) is discarded as soon as the key is placed in the map so if
	 * frequent garbage collections occur we will lose all caching benefits in a WeakHashMap implementation.
	 */
	 ...
	 * @param parameters
	 * @return An object which properly implements the equals and hashCode methods. Should be overridden if the default implementation is insufficient
	 *         for your purposes.
	 */
	protected Object getCacheKey(Class[] parameterTypes, Object[] parameters) {
	 ...
	 */
	protected final Object _getInstance(Class[] parameterTypes, Object[] parameters) {
	...
	/**
	 * Back door method for testing purposes. It will switch the factory over to a cached version and will always return the instance provided for the
	 * given parameter list until the resetFactoryDefaults method is called. (I know it sucks having this as a public method but that's life)
	 * <p />
	 ...
	 */
	public void overrideCachedInstance(Class[] parameterTypes, Object[] parameters, Object instance) {
		// make the cache a normal hashMap so that it does not get garbage collected.
		cache = new HashMap();
		setCreateNewInstanceEachTime(false);
		Object cacheKey = getCacheKey(parameterTypes, parameters);
	...
	private final Object getNewInstance(Class[] parameterTypes, Object[] parameters) {
		return ProxyAssistant.createProxiedInstance(getInstanceClass(), parameterTypes, parameters, getInvocationHandlers());
	}

	private final Object getStaticInstance(Class[] parameterTypes, Object[] parameters) {
		Object cacheKey = getCacheKey(parameterTypes, parameters);
		...
		Object newInstance = getNewInstance(parameterTypes, parameters);
